package com.bitozen.zencamp.backend.common;
...
public class MessageCode implements Serializable {
    ...
    private static final ConcurrentHashMap<String, MessageCode> _MSGMAP = new ConcurrentHashMap<String, MessageCode>();
    ...
    public static void setMessageSources(Collection<MessageSource> messageSources) {
        if (_msgSrcs == null) {
...
//                    continue;
            ...
            if (debugEnabled) {
                log.debug("****Message " + msgKey + " is not found in available message sources");
            }
        } else if (debugEnabled) {
            log.debug("****No message source available");
        }
        if (defaultMsg == null) {
            return null;
        }
